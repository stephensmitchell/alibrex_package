...
# --------------------------------------------------------------------------
# Safety helpers - AlibreX COM proxies raise on missing/stale members, so
# every read goes through ``safe`` and one bad property never aborts the scan.
# --------------------------------------------------------------------------
...
# Length units -> mm, angle units -> deg (enum names with AD_ stripped).
...
# --------------------------------------------------------------------------
# Document / material / physical / configurations
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Selection-set fingerprints (edges / faces) + topology
# --------------------------------------------------------------------------
...
def serialize_collection(coll):
    ...
    for i in range(safe(lambda: coll.Count, 0)):
        ...
        if item is None:
            continue
        if safe(lambda: item.StartVertex) is not None:        # edge
            out.append(edge_fingerprint(item))
        elif safe(lambda: item.Loops) is not None:            # face
...
# --------------------------------------------------------------------------
# Feature inference (resilient to FeatureType COMException)
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Per-type feature parameter extraction
# --------------------------------------------------------------------------
...
# Empty AlibreX interfaces ('pass' in the type stub) - the feature exists but
# its definition cannot be read at all. Still recorded, flagged "unreadable".
# (Sheet-metal features carry real data in the UI but expose nothing via API.)
_UNREADABLE = {
...
def scan_features(part, sketches):
    ...
    for i in range(n):
        ...
        # Cross-link every sketch this feature consumes (profile, sweep path,
        # loft cross-sections...). The Path/CrossSection collectors are opaque
        # proxies, so consumed_by name-matching is the reliable source.
