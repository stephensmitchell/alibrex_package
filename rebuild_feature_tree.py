...
# A base sketch plane is identified by which global axis its normal points
# along: XY->z (index 0), YZ->x (index 1), ZX->y (index 2).
        ...
        for e in sketch.get("entities", []):
            ...
            if e.get("is_reference"):
                try:
                    ...
                    pass
        # Generic geometry capture carries no sketch constraints, so endpoints
        # that were welded by coincidence constraints come back with micro-gaps
        # and a once-closed profile reads as "open". Heal disjoint ends (must
        # run inside the active sketch session) so boss/cut features can consume
        # the loop. Arcs are rebuilt by signed angle, so their computed
        # endpoints in particular need this snap.
...
def add_entity(sk, e):
    ...
    if t == "arc":
        ...
        if ang is not None:
            # Preserve winding: positive angle = CCW (right-hand rule). Using
            # center+start+end instead would lose this and can flip a major arc
            # to its minor complement -> self-intersecting profile wires.
    ...
    # bspline (only endpoints captured), text, shape-pattern: not reconstructable
    return None
...
# Features that need geometry references the generic format intentionally
# does not carry. Reported, not guessed.
_NEEDS_REFERENCES = {
...
def build_feature(part, feat, sketches_by_id, log):
    ...
    try:
        ...
        if "open sketch" in text.lower():
            # Profile failed to re-close: the generic format omits the sketch
            # constraints that originally welded the loop. Not a data error.
...
# --------------------------------------------------------------------------
# Parameters (restore equations / create free-standing ones)
# --------------------------------------------------------------------------
...
def restore_parameters(part, params, log):
    ...
    # Create any captured parameter the features didn't already produce.
    for spec in params:
        nm = spec.get("name")
    ...
    # Re-read, then set values + equations inside a transaction.
    existing = {}
    for i in range(coll.Count):
        pr = coll.Item(i)
        try:
...
def rebuild(model, part_name):
    ...
    # 1. Sketches first, keyed by their original id so features can find them.
    ...
    # 2. Features, in captured order (= regeneration order).
    for feat in model.get("features", []):
        build_feature(part, feat, sketches_by_id, feature_log)

    # 3. Restore parameter equations / free parameters.
