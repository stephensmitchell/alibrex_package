...
def quarter_round(sketch, *, r: float,
    ...
    sketch.Figures.AddLine(cx, cy, cx + r, cy)
    sketch.Figures.AddLine(cx, cy, cx,     cy + r)
...
def crown(sketch, *, h: float, projection: float, cove_r: float | None = None,
    ...
    figs = sketch.Figures
    figs.AddLine(*p_wall_bottom, *p_cove_start)
    figs.AddCircularArcByCenterStartEnd(
        cove_center[0], cove_center[1],
        p_cove_start[0], p_cove_start[1],
        p_cove_end[0],   p_cove_end[1],
    )
    figs.AddLine(*p_cove_end,    *p_top_right)
    figs.AddLine(*p_top_right,   *p_top_left)
    figs.AddLine(*p_top_left,    *p_wall_bottom)
