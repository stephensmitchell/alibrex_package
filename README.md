...
- **Full IDE support.** Every interface, enum, and method is declared in PEP 561 type stubs: autocomplete, hover docs, and type-checking work in VS Code, PyCharm, or any pyright/Pylance-aware editor.
...
- **Example and demo driven.** The repository includes Jupyter notebooks, plus test scripts, demos, and reference utilities.
...
| `examples/pdm/` | PDM (Product Data Management) walkthrough: 13 progressive scripts covering connect, safes, property defs/classes/templates, projects/libraries/folder tree, file properties plus version history, and a locked-file scan |
...
`AlibreX.dll` is not redistributed. At import time, `alibrex` locates it by checking, in order: the `$ALIBREX_DLL` env var, the Windows registry (`AlibreX.AutomationHook` COM ProgID, then `HKLM\SOFTWARE\Alibre, Inc.\Alibre Design\…\HomeDirectory`), and finally `%ProgramFiles%\Alibre Design*\Program\AlibreX.dll`.

See `notebooks/diagnose_setup.ipynb` to inspect what each source returns on your machine.

> **Note:** Treat the demo and example code as a reference and learning resource, not production code. The API can change without deprecation warnings; breaking changes go in the changelog and release notes.
