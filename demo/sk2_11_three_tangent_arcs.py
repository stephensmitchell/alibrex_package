...
def main() -> int:
    ...
    sk.BeginChange()
    try:
        # Three arcs drawn roughly nose-to-tail.
        a = sk.Figures.AddCircularArcByCenterStartEnd(
            0.0, 0.0,        # centre
            2.0, 0.0,        # start
            0.0, 2.0,        # end (90 deg CCW)
        )
        b = sk.Figures.AddCircularArcByCenterStartEnd(
            0.0, 4.0,        # centre 4 units up
            0.0, 2.0,        # start (matches previous arc's end)
            -2.0, 4.0,       # end
        )
        c = sk.Figures.AddCircularArcByCenterStartEnd(
            -4.0, 4.0,
            -2.0, 4.0,       # start (matches previous arc's end)
            -4.0, 6.0,
        )
    finally:
        sk.EndChange()
